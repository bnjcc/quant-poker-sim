...
4. **Analyze** (`/experiments/[id]`) — beginner-first results shown as a normalized win-rate percentage, with plain-English outcome cards first, visual breakdowns next, and confidence intervals, volatility, model confidence, and bankroll-risk analytics farther down. Results also include showdown vs non-showdown, position / stack depth / pot type, starting-hand heatmap, action mix, bet-size distribution, manual-vs-simulated comparison, and a replayable hand-history explorer.
5. **Simulated Hand Review** — choose any number of eligible model decisions, including quick choices above eight and the full available sample, then confirm each decision or enter the action and size you would actually choose. Corrected bets and raises start at the same two-thirds-pot suggestion used during calibration and retain the 50%, 66%, pot, and all-in shortcuts. Range-first reviews omit entire hands outside the strategy's explicit starting range. Feedback is saved in the experiment and in the dedicated `strategy_reviews` dataset. Any corrections retrain an experiment-specific policy and automatically rerun the same seeded simulation; an all-accurate round stops without rerunning. `/accuracy` summarizes the saved feedback without exposing user exports.
