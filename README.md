...
1. **Calibrate** — either play a normal sample at `/calibrate`, or use `/range-calibrate` to select an exact 169-grid first-in range and play only those starting hands. The range-first flow saves the chosen hands as a hard preflop rule while recording how you act and size bets. Every decision retains complete context: position, stack depth, pot size, bet faced, SPR, street, action history, and hole cards.
