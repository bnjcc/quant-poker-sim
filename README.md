...
1. **Calibrate** — either play a normal sample at `/calibrate`, or use `/range-calibrate` to select an exact 169-grid first-in range and play only those starting hands. Both flows give the user a 15-second action clock and show each opponent turn for 0.5 seconds, then display the opponent's simulated decision time beside the resulting action. Every decision retains position, stack depth, pot size, bet faced, SPR, street, action history, hole cards, real response time, timeout state, and the most recent opponent timing cue.
