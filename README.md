...
5. **Simulated Hand Review** — choose how many eligible model decisions to replay from different stored hands in the browser, then confirm each decision or enter the action and size you would actually choose. Range-first reviews omit entire hands outside the strategy's explicit starting range. Feedback is saved in the experiment and in the dedicated `strategy_reviews` dataset. Any corrections retrain an experiment-specific policy and automatically rerun the same seeded simulation; an all-accurate round stops without rerunning. `/accuracy` summarizes the saved feedback without exposing user exports.
