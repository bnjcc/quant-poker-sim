...
5. **Review accuracy** — replay eight model decisions from different stored hands, confirm each decision or enter the action and size you would actually choose. Feedback is saved in the experiment and in the dedicated `strategy_reviews` dataset. Any corrections retrain an experiment-specific policy and automatically rerun the same seeded simulation; an all-accurate round stops without rerunning. `/accuracy` summarizes and exports the feedback.
