    ...
    try {
      ...
      setError(
        caught instanceof Error
          ? caught.message
          : "Could not load previous experiments.",
      );
      setExps([]);
      setStrategies([]);
    }
        ...
        <table className="w-full min-w-[900px] text-sm">
          <thead>
            <tr className="text-left border-b border-line">
              ...
              <th className="label px-4 py-3 font-normal text-right">
                Win rate
              </th>
          ...
          <tbody>
            {exps.map((e) => {
              ...
              return (
                <tr
                  key={e.id}
                  className="border-b border-line last:border-0 hover:bg-panel2/50"
                >
                  ...
                  <td className="px-4 py-3 text-xs">
                    {strategy ? (
                      <Link
                        href={`/profile?strategy=${encodeURIComponent(strategy.id)}`}
                      ...
                      <span
                        ...
                        title="The saved strategy is no longer available, but this run keeps its original strategy name."
                      >
                        {strategyName}
                      </span>
                    )}
                  ...
                  <td className="px-4 py-3 text-right mono">
                    {r
                      ...
                      : e.config.hands.toLocaleString()}
                  </td>
                  <td
                    className="px-4 py-3 text-right mono font-bold"
                    style={{
                      color: r
                        ? r.bb100 >= 0
                          ? "var(--gain)"
                  ...
                  <td className="px-4 py-3 text-right whitespace-nowrap">
                    <Link
                      href={`/experiments/${e.id}`}
                      className="btn btn-primary text-xs px-2 py-1 mr-2"
                    >
                      View
                    </Link>
                    <Link
                      href={`/experiments/new?duplicate=${e.id}`}
                      className="btn text-xs px-2 py-1 mr-2"
                    >
                      Duplicate
                    </Link>
                    <button
