...
export default function ProfilePage() {
    ...
    getStore()
      ...
      .then((c) => {
        ...
        if (c.length) {
          ...
          const initial =
            c.find((strategy) => strategy.id === requested) ?? c[0];
  ...
  const timedDecisions = cal.decisions.filter(
    (decision) =>
      ...
      Number.isFinite(decision.responseTimeMs),
  );
  const sortedTimes = timedDecisions
    .map((decision) => decision.responseTimeMs)
    .sort((a, b) => a - b);
  ...
  const medianTime = sortedTimes.length
    ...
    : null;
  const snapCount = timedDecisions.filter(
    (decision) => decision.responseTimeMs <= SNAP_DECISION_MS,
  ).length;
  const timeoutCount = timedDecisions.filter(
    ...
    <div>
      ...
      <section className="panel px-5 py-4 mb-4 max-w-3xl">
            ...
            >
              {cals.map((strategy) => (
                <option key={strategy.id} value={strategy.id}>
                  {strategy.method === "range-first"
                    ? `[Range-first] ${strategy.name}`
                    : strategy.name}
                </option>
              ))}
            </select>
        ...
        <div className="flex flex-wrap items-end gap-2 mt-3 pt-3 border-t border-line">
          ...
          <button
            className="btn"
            onClick={renameStrategy}
            disabled={savingName || draftName.trim() === cal.name}
          >
        ...
        <div className="text-xs text-muted mt-2">
          {message ??
            `${cals.length} saved strateg${cals.length === 1 ? "y" : "ies"} in this account.`}
        </div>
      </section>
      ...
      {small && (
        <div className="mb-4">
          <WarningNote>
            Only {cal.handsPlayed} calibration hands.{" "}
            {rangeFirst ? "Betting estimates" : "Estimates below"} have wide
            intervals and the simulated model will lean heavily on its prior.{" "}
            {RELIABLE_SAMPLE_THRESHOLD}+ hands are recommended before trusting
            the profile.
          </WarningNote>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <Stat
          label={rangeFirst ? "Betting samples" : "Calibration hands"}
          value={String(cal.handsPlayed)}
          sub={`${cal.decisions.length} decisions`}
        />
        {rangeFirst ? (
          <>
            <Stat
          ...
          <>
            <Stat
              label="VPIP"
              value={fmtPct(t.vpip.value, 1)}
              sub="voluntarily put money in"
            />
            <Stat
              label="PFR"
              value={fmtPct(t.pfr.value, 1)}
              sub="preflop raise"
            />
          </>
        )}
      ...
      {timedDecisions.length > 0 && (
        <>
          ...
          <p className="text-xs text-muted mb-6 max-w-3xl">
            The simulated policy samples your observed response times by action
            and situation. Its action frequencies also condition on whether the
            previous opponent action was a snap, normal decision, or tank when
            enough matching observations exist.
          </p>
        </>
      )}
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <section className="panel px-5 py-4">
          ...
          <TendencyRow
            label={
              rangeFirst
                ? "VPIP — play when selected"
                : "VPIP — voluntarily play"
            }
            stat={t.vpip}
            hint="Share of hands where you voluntarily put chips in preflop"
          />
          <TendencyRow
            label={
              rangeFirst ? "PFR — raise when selected" : "PFR — preflop raise"
            }
            stat={t.pfr}
            hint="Share of hands you raised preflop"
          />
          <TendencyRow
            label="3-bet — preflop re-raise"
            stat={t.threeBet}
            hint="Re-raise frequency when facing one raise"
          />
          <TendencyRow
            label="Fold to 3-bet"
            stat={t.foldToThreeBet}
            hint="Fold frequency after raising and facing a 3-bet"
          />
          <TendencyRow
            label="Open-limp"
            stat={t.limp}
            hint="Entering unraised pots with a call instead of a raise"
          />
        </section>
        ...
        <section className="panel px-5 py-4">
          ...
          <TendencyRow
            ...
            stat={t.cbet}
            hint="Betting the flop after raising preflop"
          />
          <TendencyRow
            label="Fold to c-bet"
            stat={t.foldToCbet}
            hint="Folding the flop when facing a continuation bet"
          />
          <TendencyRow
            label="Raise vs bet (check-raise proxy)"
            stat={t.checkRaise}
            hint="Raising when facing a postflop bet"
          />
          <TendencyRow
            label="River call"
            stat={t.riverCall}
            hint="Calling frequency when facing a river bet"
          />
          <TendencyRow
            label="Aggression frequency"
            stat={t.aggressionFrequency}
            hint="Share of all decisions that were bets or raises"
          />
        </section>
        ...
        <section className="panel px-5 py-4">
          ...
          <TendencyRow
            ...
            stat={t.wentToShowdown}
            hint="Reaching showdown after seeing a flop"
          />
          <TendencyRow
            label="Won money at showdown"
            stat={t.wonAtShowdown}
            hint="Winning share of showdowns reached"
          />
          <TendencyRow
            label="Late-street bluff proxy"
            stat={t.bluffProxy}
      ...
      <div className="mt-8">
        <AnalyticsGlossary
          groups={["poker", "positions", "model"]}
          title="Explain the detailed strategy statistics"
        />
      </div>

      <div className="text-xs text-muted mt-6 max-w-3xl">
        How to read this: the model behind simulation is a bucketed behavioral
        policy (street × position × situation × hand strength × opponent
        timing), shrunk toward a strength-aware prior when a bucket has few
        samples. The percentage bars mirror the displayed tendency. Read them
        together with the opportunity count and 95% interval to judge how much
        evidence supports each estimate.
      </div>
    </div>
  );
}
