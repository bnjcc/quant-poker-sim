...
export default function SettingsPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (store.mode === "supabase") {
        ...
        setLocalCount(
          local.calibrations + local.experiments + local.hands + local.reviews,
        );
      }
    } catch (error) {
      setMessage(
  ...
  const deleteCal = async (id) => {
    const linkedExperiments = exps.filter(
      (experiment) => experiment.calibrationId === id,
    ).length;
    const impact =
      linkedExperiments > 0
    ...
    try {
      ...
      setMessage(
        error instanceof Error
          ? error.message
          : "Could not delete the calibration.",
      );
    }
  };
  const wipe = async () => {
    if (
      !confirm(
        "Delete ALL calibrations, experiments, and stored hands? This cannot be undone.",
      )
    )
      return;
    try {
      ...
      setMessage(
        error instanceof Error ? error.message : "Could not delete the data.",
      );
    }
  };
  ...
  return (
    <div>
      ...
      />
      {message && (
        <div className="panel px-4 py-3 text-sm mb-4 max-w-4xl">{message}</div>
      )}
      <div className="grid lg:grid-cols-2 gap-4 max-w-4xl">
        <ThemeSelector />
        <section className="panel px-5 py-4">
          <h2 className="font-semibold mb-2">
            {summary?.mode === "supabase" ? "Cloud storage" : "Browser storage"}
          </h2>
          <div className="text-sm text-muted space-y-1">
            <div>
              Storage:{" "}
              <span className="mono text-ink">
                {summary?.usageLabel ?? "Loading…"}
              </span>
            </div>
            <div>
              Saved strategies:{" "}
            ...
            <div>
              Experiments:{" "}
              <span className="mono text-ink">{summary?.experiments ?? 0}</span>
            </div>
            <div>
              Saved hands:{" "}
              <span className="mono text-ink">{summary?.hands ?? 0}</span>
            </div>
            <div>
              Strategy reviews:{" "}
              <span className="mono text-ink">{summary?.reviews ?? 0}</span>
            </div>
          </div>
          ...
          {summary?.mode === "supabase" && localCount > 0 && (
            <div className="mt-4 rounded-md border border-line bg-panel2 px-3 py-3">
              ...
              <p className="text-xs text-muted mt-1">
                Import items that are not already in this account. Existing
                cloud records are never overwritten, and the local backup is
                retained.
              </p>
              <button
                className="btn mt-3"
          ...
          <button className="btn btn-danger mt-4" onClick={wipe}>
            Delete all {summary?.mode === "supabase" ? "cloud" : "browser"} data
          </button>
        </section>
        <section className="panel px-5 py-4">
                ...
                >
                  <div className="min-w-0">
                    ...
                    <div className="text-[11px] text-muted mono">
                      {calibration.handsPlayed} hands ·{" "}
                      {calibration.decisions.length} decisions
                    </div>
                  </div>
                  <Link
                    className="btn text-xs px-2 py-1 ml-auto"
                    href={`/profile?strategy=${encodeURIComponent(calibration.id)}`}
                  >
                    View
                  </Link>
                  <button
                    className="btn btn-danger text-xs px-2 py-1"
                    onClick={() => deleteCal(calibration.id)}
                  >
                    Delete
                  </button>
                </div>
          ...
          <div className="text-xs text-muted mt-3">
            {exps.length} experiment{exps.length === 1 ? "" : "s"} in this
            storage mode.
          </div>
        </section>
      </div>
