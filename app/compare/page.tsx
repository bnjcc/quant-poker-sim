...
export default function ComparePage() {
  ...
  const rows = useMemo(() => (exps ?? []).filter((e) => picked.includes(e.id)), [exps, picked]);
  ...
  const metrics: { label: string; get: (e: Experiment) => string; tone?: (e: Experiment) => string }[] = [
    ...
    { label: "Rake paid (bb)", get: (e) => (e.results!.rakePaid / e.config.table.bigBlind).toFixed(0) },
    { label: "Seed", get: (e) => e.config.seed },
    { label: "Pool skill ×", get: (e) => e.config.pool.skillShift.toFixed(2) },
    { label: "Rake", get: (e) => `${(e.config.table.rake.percentage * 100).toFixed(1)}% cap ${e.config.table.rake.cap}` },
  ];
            ...
            <tbody>
              {metrics.map((m) => (
                <tr key={m.label} className="border-b border-line last:border-0">
                  ...
                  {rows.map((e) => (
                    <td key={e.id} className="px-4 py-2.5 mono" style={{ color: m.tone?.(e) }}>
                      {m.get(e)}
                    </td>
                  ))}
                </tr>
