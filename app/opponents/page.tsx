  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      ...
      <div className="h-1.5 flex-1 rounded bg-panel2 overflow-hidden">
        <div className="h-full rounded bg-info" style={{ width: `${Math.min(100, (value / max) * 100)}%` }} />
      </div>
      <span className="mono w-10 text-right">{max === 1 ? fmtPct(value) : value.toFixed(1)}</span>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
        {AGENT_PRESETS.map((p) => (
          <div key={p.id} className="panel px-5 py-4">
            ...
            <p className="text-xs text-muted mt-1 mb-3 min-h-[2.2rem]">{p.description}</p>
            ...
            <div className="mono text-[11px] text-muted mt-3">
              sizes ~{Math.round(p.betSizeMean * 100)}% pot ±{Math.round(p.betSizeStd * 100)} · buy-in {p.buyInBB}bb
      ...
      <p className="text-xs text-muted mt-6 max-w-3xl">
        Honest caveat: these are heuristic policies, not learned models of real populations. They produce plausible
        aggregate statistics (VPIP/PFR/AF in realistic ranges) but simpler postflop play than strong humans.
      </p>
    </div>
  );
}
