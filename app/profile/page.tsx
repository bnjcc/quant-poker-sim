  ...
  return (
    <div>
      <PageHeader
        ...
        sub="Estimated from your calibration play. Every figure is a smoothed frequency with a 95% Wilson interval — not a fixed label."
        right={
      ...
      {small && (
        <div className="mb-4">
          <WarningNote>
            Only {cal.handsPlayed} calibration hands. Estimates below have wide intervals and the simulated model will
            lean heavily on its prior. {RELIABLE_SAMPLE_THRESHOLD}+ hands are recommended before trusting the profile.
          </WarningNote>
        </div>
      )}

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <Stat label="Calibration hands" value={String(cal.handsPlayed)} sub={`${cal.decisions.length} decisions`} />
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <section className="panel px-5 py-4">
          <h2 className="font-semibold mb-2">Preflop tendencies</h2>
          <TendencyRow label="VPIP" stat={t.vpip} hint="Share of hands where you voluntarily put chips in preflop" />
          <TendencyRow label="Preflop raise (PFR)" stat={t.pfr} hint="Share of hands you raised preflop" />
