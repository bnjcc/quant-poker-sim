...
New games default to 1/3 chips, with 2/5 available as a preset in both calibration flows and experiment setup. Before range editing, range-first calibration also chooses a 6-player or 9-player table; positional charts and required ranges are restricted to positions that exist at that table size. Calibration bet controls retain chip-denominated stacks and amounts, expose a three-big-blind preflop shortcut (9 chips at 1/3 or 15 chips at 2/5), and use the selected small blind as the slider and step-button increment to match PokerStars' fine-grained incremental adjustment mode. Direct numeric entry remains available for an exact legal chip amount.
