...
export function sampleAgentDecisionTiming(profile, ctx, action, rng) {
  ...
  if (rng.chance(0.003 * (1 - profile.skill))) {
    return { decisionTimeMs: OPPONENT_ACTION_CLOCK_MS, timedOut: true };
  }
  ...
  if (rng.chance(0.05 + profile.skill * 0.035)) {
    return {
      decisionTimeMs: Math.round(
        TANK_DECISION_MS +
          rng.next() * (OPPONENT_ACTION_CLOCK_MS - TANK_DECISION_MS - 350),
      ),
      timedOut: false,
    };
  }
...
function preflopDecision(profile, ctx, rng) {
  ...
  if (!ctx.facedRaisePreflop) {
    ...
    const preflopPressure = profile.preflopPressure ?? 0;
    const pressureRaise =
      preflopPressure > 0 &&
      noisy >= playThreshold &&
      rng.chance(preflopPressure);
    if ((strongEnoughToRaise || pressureRaise) && !rng.chance(profile.limp)) {
  ...
  // Selective pressure profiles can 3-bet hands near the top of their normal
  // defending range. The hand-strength gate prevents indiscriminate aggression.
  const preflopPressure = profile.preflopPressure ?? 0;
  if (
    preflopPressure > 0 &&
    noisy > defendThreshold - 0.08 &&
    rng.chance(preflopPressure * profile.threeBet * 0.35) &&
    ctx.legal.types.includes("raise")
  ) {
    return { type: "raise", toAmount: sizeBet(profile, ctx, rng, true) };
  }
