const Z = 1.96;
export function freqStat(numerator, denominator, prior = 0.5, priorWeight = 2) {
 ...
 */
export function computeTendencies(decisions, hands, userSeatByHand) {
  ...
  for (const [handNo, ds] of byHand) {
    ...
    if (pre.length > 0) {
      ...
      posVpip.set(pos, pv);
      if (pre.some((d) => d.action.type === "raise" || d.action.type === "bet"))
        pfrN++;
      // First voluntary preflop entry into an unraised pot: limp vs raise.
      const firstIn = pre.find(
        (d) =>
          !d.context.facedRaisePreflop &&
          d.action.type !== "fold" &&
          d.action.type !== "check",
      );
      if (firstIn) {
      ...
      const face1 = pre.filter(
        (d) =>
          d.context.facedRaisePreflop && d.context.numRaisesThisStreet === 1,
      );
      ...
      const face3b = pre.filter(
        (d) =>
          d.context.numRaisesThisStreet >= 2 && d.context.isPreflopAggressor,
      );
    ...
    for (const d of ds) {
      if (d.action.type === "bet" || d.action.type === "raise") {
        betsRaises++;
        if (d.potFraction !== null && d.context.street !== "preflop")
          sizings.push(d.potFraction);
      } else if (d.action.type === "call") calls++;
      else passive++;
      if (
        d.context.street === "flop" &&
        d.context.isPreflopAggressor &&
        d.context.betFaced === 0
      ) {
        cbetD++;
      ...
      if (
        (d.context.street === "turn" || d.context.street === "river") &&
  ...
  for (const h of hands) {
    ...
    const sawFlop =
      h.board.length >= 3 &&
      h.actions.some((a) => a.seat === seat && a.street !== "preflop");
    if (sawFlop) {
      wtsdD++;
    ...
    if (
      res.showedDown &&
      res.handRank &&
      res.handRank.category === "high-card"
    ) {
      const lateAggr = h.actions.some(
        (a) =>
          a.seat === seat &&
          (a.street === "turn" || a.street === "river") &&
          (a.type === "bet" || a.type === "raise"),
      );
      if (lateAggr) bluffN++;
    }
  }
  const mean = sizings.length
    ? sizings.reduce((a, b) => a + b, 0) / sizings.length
    : 0.66;
  const std =
    sizings.length > 1
      ? Math.sqrt(
          sizings.reduce((a, b) => a + (b - mean) ** 2, 0) /
            (sizings.length - 1),
        )
      : 0.2;
  return {
    ...
    aggressionFrequency: freqStat(
      betsRaises,
      betsRaises + calls + passive,
      0.3,
    ),
