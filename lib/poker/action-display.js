...
/** Keep opponents hidden unless that exact seat reached showdown. */
export function visibleHoleCardsForSeat(player, userSeat, results = []) {
  if (player.seat === userSeat) return player.holeCards;
  const result = results.find((entry) => entry.seat === player.seat);
  return result?.showedDown ? (result.holeCards ?? player.holeCards) : null;
}
