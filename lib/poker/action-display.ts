import { Action, Street } from "@/types/poker";
 ...
 */
export function visibleActionBySeat(actions: Action[], street: Street): Map<number, Action> {
  ...
  const visible = new Map<number, Action>();
  for (let index = lastAggressiveIndex < 0 ? 0 : lastAggressiveIndex; index < streetActions.length; index++) {
    const action = streetActions[index];
