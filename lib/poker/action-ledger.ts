import { ActionType, HandHistory, Position, Street } from "@/types/poker";

export interface HandActionLedgerEntry {
  index: number;
  street: Street;
  seat: number;
  playerId: string;
...
export function buildHandActionLedger(hand: HandHistory): HandActionLedgerEntry[] {
  ...
  return hand.actions.map((action, index) => {
    ...
    if (!player) throw new Error(`Hand action references missing seat ${action.seat}`);
    ...
    switch (action.type) {
      ...
      case "raise":
        ...
        break;
      case "all-in":
        label = totalAfter > highestBefore
