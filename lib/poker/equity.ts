...
export function estimateEquity(
  ...
  for (let it = 0; it < iterations; it++) {
    ...
    let d = 0;
    const oppHands: Card[][] = [];
    for (let o = 0; o < numOpponents; o++) oppHands.push([deck[d++], deck[d++]]);
...
export function preflopStrength(hole: Card[]): number {
  ...
  const rankScore = (r: number) => (r >= 11 ? r - 4.5 : r / 2); // A=9.5? tweak below
  let score: number;
  const high = a.rank === 14 ? 10 : a.rank === 13 ? 8 : a.rank === 12 ? 7 : a.rank === 11 ? 6 : a.rank / 2;
  void rankScore;
  if (a.rank === b.rank) {
