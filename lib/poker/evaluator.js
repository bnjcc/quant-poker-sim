...
function straightHigh(sortedDescUnique) {
  ...
  for (const high of sortedDescUnique) {
    if (
      high >= 6 &&
      [high, high - 1, high - 2, high - 3, high - 4].every((r) => set.has(r))
    ) {
      return high;
    }
  }
...
function straightFromMask(mask) {
  // Ace-low support.
...
function score5(cat, r1, r2 = 0, r3 = 0, r4 = 0, r5 = 0) {
...
export function fastScore7(cards) {
  ...
  if (pair1 && pair2) {
    let kicker = 0;
    for (let r = 14; r >= 2; r--) {
      if (r !== pair1 && r !== pair2 && fastCounts[r] > 0) {
        kicker = r;
        break;
      }
    }
...
export function sortedRanks(cards) {
