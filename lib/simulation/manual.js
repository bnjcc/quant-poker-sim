...
export class ManualSession {
  ...
  step(paceOpponents = false) {
    ...
    if (!this.current) {
      const cardsForRange = (range) => {
        if (!range) return undefined;
        const handIndex = this.session.rng.weighted(
          range.map(startingHandCombos),
        );
        return cardsForStartingHand(range[handIndex], this.session.rng);
      };
      const userHoleCards = this.startingHandsByPosition
        ? (position) => {
            ...
            return cardsForRange(range);
          }
        : cardsForRange(this.startingHands);
