...
export class ManualSession {
  ...
  constructor(opts) {
    ...
    const completed = opts.completedProgress;
    if (completed) {
      this.decisions = [...(completed.decisions ?? [])];
      this.histories = [...(completed.histories ?? [])];
      this.userSeatByHand = new Map(
        Object.entries(completed.userSeatByHand ?? {}).map(
          ([handNumber, seat]) => [Number(handNumber), seat],
        ),
      );
      this.handsPlayed = Math.min(
        this.targetHands,
        Math.max(0, Number(completed.handsPlayed) || 0),
      );
      // Resumed sessions start a fresh table, but hand numbers must remain
      // unique so histories and per-hand user seats continue to line up.
      this.session.handNumber = Math.max(
        this.handsPlayed,
        ...this.histories.map((history) => history.handNumber ?? 0),
      );
    }
