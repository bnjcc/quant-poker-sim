...
import { BehavioralPolicy, SerializedPolicy } from "@/lib/player-model/policy";
import { Aggregator, SimulationAggregates } from "@/lib/analytics/aggregate";
import { PoolConfig, TableSession, policyDecider } from "./table";
...
export interface SimulationSettings {
  ...
  pool: PoolConfig;
  userBuyInBB: number;
  /** "detailed" stores all hand histories; "high-speed" stores aggregates + samples. */
  mode: "detailed" | "high-speed";
  /** In high-speed mode, keep 1 in N full histories. */
  sampleEvery: number;
}
...
export interface SimulationOutput {
  ...
  hands: HandHistory[]; // all (detailed) or sampled (high-speed)
  userDecisionLog: TableSession["userDecisionLog"];
  cancelled: boolean;
}
...
export async function runSimulation(
  ...
  for (let i = 0; i < settings.hands; i++) {
    ...
    if ((i + 1) % chunkSize === 0) {
      onProgress?.({ handsDone: i + 1, handsTotal: settings.hands, userNet: agg.snapshot().userNet });
      // Yield to the event loop so the UI never blocks.
      await new Promise((r) => setTimeout(r, 0));
    }
  }
  onProgress?.({ handsDone: agg.snapshot().totalHands, handsTotal: settings.hands, userNet: agg.snapshot().userNet });
