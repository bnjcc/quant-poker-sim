...
export class TableSession {
  ...
  constructor(opts) {
    ...
    if (configuredPolicyPlayers.length > seats.length) {
      throw new Error("More real-user strategies were supplied than table seats.");
    }
  ...
  manageTurnover() {
    ...
    if (this.primaryParticipantId) {
      const primarySeat = this.participantSeatById.get(this.primaryParticipantId);
      const primary = this.policyPlayers.get(primarySeat);
      if (primary) {
        this.userStack = primary.stack;
        this.userBuyIns = primary.buyIns;
      }
    } else if (this.userSeat !== null && this.userStack < this.config.bigBlind) {
  ...
  beginHand(includeUser, userName = "You", manual = false, userHoleCards) {
    ...
    const engine = new HandEngine({
      ...
      forcedHoleCards:
        userHoleCards && this.userSeat !== null
          ? { [this.userSeat]: userHoleCards }
          : undefined,
    });
