...
export class LocalStorageStore {
  ...
  async listCalibrations() {
    return this.read(CAL_KEY, []).filter((calibration) => !calibration.draft);
  }
  async listCalibrationRecords() {
    return this.read(CAL_KEY, []);
  }
  async getCalibrationDraft(method) {
    return (
      (await this.listCalibrationRecords()).find(
        (calibration) => calibration.draft && calibration.method === method,
      ) ?? null
    );
  }
  async saveCalibrationDraft(calibration) {
    await this.saveCalibration({ ...calibration, draft: true });
  }
  async deleteCalibrationDraft(method) {
    const draft = await this.getCalibrationDraft(method);
    if (draft) await this.deleteCalibration(draft.id);
  }
  ...
  async saveCalibration(calibration) {
    const all = (await this.listCalibrationRecords()).filter(
  ...
  async deleteCalibration(id) {
    this.write(
      CAL_KEY,
      (await this.listCalibrationRecords()).filter((item) => item.id !== id),
    );
  }
...
export class SupabaseStore {
  ...
  async listCalibrations() {
    ...
    return (data ?? [])
      .map(calibrationFromRow)
      .filter((calibration) => !calibration.draft);
  }
  async getCalibrationDraft(method) {
    const { data, error } = await this.client
      .from("calibrations")
    ...
    throwIfError("Load calibration progress", error);
    return (
      (data ?? [])
        .map(calibrationFromRow)
        .find(
