...
import { HandEngine, HandSetupPlayer } from "@/lib/poker/engine";
import { cardsFromString } from "@/lib/poker/deck";
import { Rng } from "@/lib/poker/rng";
import { TableConfig, Card } from "@/types/poker";
...
function players(stacks: number[]): HandSetupPlayer[] {
  return stacks.map((stack, i) => ({ seat: i, playerId: `p${i}`, name: `P${i}`, stack }));
}
 ...
 */
function riggedDeck(holePerSeatInDealOrder: string[], board: string): Card[] {
  const deck: Card[] = [];
  ...
  const full = [
    ...dealSequence,
    burn[0], boardCards[0], boardCards[1], boardCards[2], // flop
    burn[0], boardCards[3], // turn
    burn[0], boardCards[4], // river
  ];
...
describe("blinds and rotation", () => {
  it("posts SB left of button, BB next, and action starts UTG (3+ handed)", () => {
    const e = new HandEngine({ players: players([100, 100, 100]), buttonSeat: 0, config, rng: new Rng(1), handNumber: 1 });
    const posts = e.actions.filter((a) => a.type === "post-sb" || a.type === "post-bb");
  ...
  it("heads-up: button posts SB and acts first preflop", () => {
    const e = new HandEngine({ players: players([100, 100]), buttonSeat: 0, config, rng: new Rng(1), handNumber: 1 });
...
describe("legal actions and min-raise", () => {
  it("enforces minimum raise size", () => {
    const e = new HandEngine({ players: players([200, 200, 200]), buttonSeat: 0, config, rng: new Rng(1), handNumber: 1 });
  ...
  it("all-in below min-raise does not grant an illegal re-raise threshold", () => {
    ...
    const e = new HandEngine({ players: players([200, 7, 200]), buttonSeat: 0, config, rng: new Rng(1), handNumber: 1 });
    e.applyAction(0, { type: "raise", toAmount: 6 });
...
describe("side pots and split pots", () => {
  it("builds correct side pots for a three-way all-in with unequal stacks", () => {
    const pots = HandEngine.buildPots([
      { seat: 0, totalCommitted: 100, folded: false } as never,
      { seat: 1, totalCommitted: 50, folded: false } as never,
      { seat: 2, totalCommitted: 100, folded: false } as never,
      { seat: 3, totalCommitted: 20, folded: true } as never,
    ]);
...
describe("fold-out and rake", () => {
  it("awards the pot uncontested and takes no rake preflop with no-flop-no-drop", () => {
    const e = new HandEngine({ players: players([100, 100, 100]), buttonSeat: 0, config: { ...config, rake: { percentage: 0.05, cap: 5, noFlopNoDrop: true } }, rng: new Rng(2), handNumber: 1 });
    e.applyAction(0, { type: "raise", toAmount: 6 });
  ...
  it("takes capped rake on a raked flop pot", () => {
    const rakeCfg: TableConfig = { ...config, rake: { percentage: 0.05, cap: 3, noFlopNoDrop: true } };
    const e = new HandEngine({ players: players([500, 500]), buttonSeat: 0, config: rakeCfg, rng: new Rng(3), handNumber: 1 });
...
describe("chip conservation", () => {
  it("total chips are conserved (minus rake) across many random hands", () => {
    ...
    for (let trial = 0; trial < 50; trial++) {
      ...
      const e = new HandEngine({ players: players(stacks), buttonSeat: trial % 6, config, rng, handNumber: trial });
      let guard = 0;
      while (!e.complete && guard++ < 200) {
        const seat = e.currentSeat!;
        const legal = e.getLegalActions(seat);
        const pick = legal.types[rng.int(legal.types.length)];
        if (pick === "bet") e.applyAction(seat, { type: "bet", toAmount: Math.min(legal.minBet + rng.int(20), legal.maxBetTo) });
        else if (pick === "raise") e.applyAction(seat, { type: "raise", toAmount: Math.min(legal.minRaiseTo + rng.int(30), legal.maxBetTo) });
