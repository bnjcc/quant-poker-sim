...
describe("starting-hand ranges", () => {
  it("builds the full 169-hand matrix covering all 1,326 combinations", () => {
    ...
    expect(new Set(ALL_STARTING_HANDS.map((hand) => hand.notation)).size).toBe(169);
    expect(rangeComboCount(ALL_STARTING_HANDS.map((hand) => hand.notation))).toBe(1326);
  ...
  it("turns range notation into matching concrete cards", () => {
    ...
    for (const notation of ["AA", "AKs", "T9o", "54s"]) {
      for (let sample = 0; sample < 20; sample++) {
        expect(holeNotation(cardsForStartingHand(notation, rng))).toBe(notation);
      }
    }
  });
  ...
  it("can force a user's cards without duplicating them in the deck", () => {
    ...
    expect(engine.players.find((player) => player.seat === 0)?.holeCards).toEqual(forced);
    const allCards = engine.players.flatMap((player) => player.holeCards ?? []);
    expect(new Set(allCards.map((card) => `${card.rank}${card.suit}`)).size).toBe(4);
  });
  ...
  it("deals every range-first calibration hand from the selected range", () => {
    ...
    for (const history of session.histories) {
      expect(selected.has(holeNotation(history.holeCards[session.userSeat]))).toBe(true);
    }
  });
});
...
describe("explicit range policy", () => {
  const baseContext: DecisionContext = {
    ...
    legal: { types: ["fold", "call", "raise"], callAmount: 2, minBet: 0, minRaiseTo: 6, maxBetTo: 200 },
    bigBlind: 2,
  };
  ...
  it("folds excluded first-in hands and always continues selected hands", () => {
    ...
    const selectedContext = { ...baseContext, holeCards: cardsFromString("AsKs") };
    ...
    expect(Object.values(probabilities.probs).reduce((sum, value) => sum + value, 0)).toBeCloseTo(1, 8);

    const restored = BehavioralPolicy.deserialize(policy.serialize());
    expect(restored.sample(baseContext, new Rng("restored")).action.type).toBe("fold");
  });

  it("does not treat the first-in range as a response-to-raise chart", () => {
    const policy = new BehavioralPolicy().setPreflopRange(["AKs"]);
    const facingRaise = { ...baseContext, facedRaisePreflop: true, numRaisesThisStreet: 1 };
