...
function runHands(seed: string, n: number) {
  const rng = new Rng(seed);
  const session = new TableSession({ config: DEFAULT_TABLE, pool, rng, userBuyInBB: 100, seatUser: true });
...
describe("player turnover", () => {
  it("replaces departing players and the table keeps running", () => {
    ...
    const session = new TableSession({ config: DEFAULT_TABLE, pool: churnPool, rng, userBuyInBB: 100, seatUser: true });
    const initialIds = new Set([...session.agents.values()].map((a) => a.playerId));
    const decider = policyDecider(new BehavioralPolicy());
    for (let i = 0; i < 300; i++) session.playHand(decider);
    const finalIds = new Set([...session.agents.values()].map((a) => a.playerId));
...
describe("runner + aggregation", () => {
    ...
    const out = await runSimulation(
      {
        ...
        pool: DEFAULT_POOL_SETTINGS ? buildPoolConfig(DEFAULT_POOL_SETTINGS) : pool,
        userBuyInBB: 100,
        mode: "detailed",
        sampleEvery: 1,
      },
  ...
  it("cancellation stops early", async () => {
    let calls = 0;
    const out = await runSimulation(
      { hands: 5000, seed: "cancel", config: DEFAULT_TABLE, pool, userBuyInBB: 100, mode: "high-speed", sampleEvery: 50 },
...
describe("manual calibration session", () => {
  it("pauses at user turns, records decisions, and completes hands", () => {
    const ms = new ManualSession({ config: DEFAULT_TABLE, pool, seed: "manual", targetHands: 5, userBuyInBB: 100 });
    let guard = 0;
    while (guard++ < 2000) {
      const step = ms.step();
      ...
      if (step.kind === "awaiting-user") {
        ...
        const action = legal.types.includes("check") ? { type: "check" as const } : { type: "fold" as const };
...
describe("player-model statistics", () => {
  ...
  it("computes VPIP/PFR from decisions: folder has ~0, raiser has ~1", () => {
    const ms = new ManualSession({ config: DEFAULT_TABLE, pool, seed: "stats", targetHands: 20, userBuyInBB: 100 });
    let guard = 0;
    while (guard++ < 5000) {
      const step = ms.step();
      if (step.kind === "session-complete") break;
...
describe("behavioral policy", () => {
  it("outputs legal, normalized probabilities and confidence grows with data", () => {
    const ms = new ManualSession({ config: DEFAULT_TABLE, pool, seed: "policy", targetHands: 30, userBuyInBB: 100 });
    let guard = 0;
    while (guard++ < 8000) {
      const step = ms.step();
      if (step.kind === "session-complete") break;
      if (step.kind === "awaiting-user") {
        ...
        const action = legal.types.includes("raise")
          ? { type: "raise" as const, toAmount: legal.minRaiseTo }
          : legal.types.includes("bet")
            ? { type: "bet" as const, toAmount: Math.max(legal.minBet, 2) }
    ...
    expect(trainedAgg).toBeGreaterThanOrEqual(emptyExplain.probs.raise + emptyExplain.probs.bet - 0.001);
...
describe("recorded decision shape", () => {
  it("captures the full required context fields", () => {
    const ms = new ManualSession({ config: DEFAULT_TABLE, pool, seed: "ctx", targetHands: 2, userBuyInBB: 100 });
    let d: RecordedDecision | null = null;
    let guard = 0;
    while (guard++ < 1000) {
      const step = ms.step();
      if (step.kind === "session-complete") break;
      if (step.kind === "awaiting-user") {
        ms.submitUserAction(step.context, step.context.legal.types.includes("check") ? { type: "check" } : { type: "fold" });
    ...
    const c = d!.context;
    for (const key of [
      "street", "position", "holeCards", "board", "potSize", "betFaced",
      "effectiveStack", "stackToPotRatio", "activePlayers", "legal", "bigBlind",
    ] as const) {
