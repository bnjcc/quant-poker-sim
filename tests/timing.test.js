...
describe("online action timing", () => {
  it("never permits a bot to check while facing a bet", () => {
    ...
    expect(normalizeActionToLegal({ type: "check" }, legal)).toEqual({
      type: "call",
    });
    ...
    const stackBeforeCall = engine.players.find(
      (player) => player.seat === 1,
    ).stack;
    tracker.apply(1, { type: "check" });
    const applied = engine.actions.at(-1);
    expect(applied.type).toBe("call");
    expect(engine.players.find((player) => player.seat === 1).stack).toBe(
      stackBeforeCall - applied.amount,
    );
  });
  it("removes stale check labels from every opponent still facing a bet", () => {
    ...
    expect(
      [...facingBet.entries()].map(([seat, action]) => [seat, action.type]),
    ).toEqual([[0, "bet"]]);
    const afterFirstResponse = visibleActionBySeat(
      [
        ...actions,
        { seat: 1, type: "call", amount: 10, street: "flop", allIn: false },
      ],
      "flop",
    );
    expect(
      [...afterFirstResponse.entries()].map(([seat, action]) => [
        seat,
        action.type,
      ]),
    ).toEqual([
  ...
  it("records only legal checks throughout manual calibration hands", () => {
    ...
    while (guard++ < 1_000) {
      ...
      if (legal.types.includes("bet")) {
        session.submitUserAction(step.context, {
          type: "bet",
          toAmount: legal.minBet,
        });
    ...
    for (const history of session.histories) {
      ...
      let priceSetter = null;
      let committedBySeat = new Map();
      for (const action of history.actions) {
        ...
        if (committedAfter > highestCommitment) {
          ...
          priceSetter =
            action.type === "post-sb" || action.type === "post-bb"
              ? null
              : action.seat;
        }
      }
  ...
  it("check/folds the rest of one hand, records intentional actions, and resets next hand", () => {
    ...
    for (let guard = 0; guard < 500 && !completedActivatedHand; guard++) {
      ...
      if (
        step.kind === "hand-complete" &&
        step.history.handNumber === activatedHand
      ) {
    ...
    expect(
      checkFoldDecisions.every((decision) =>
        ["check", "fold"].includes(decision.action.type),
      ),
    ).toBe(true);
    expect(checkFoldDecisions[0]).toMatchObject({
  ...
  it("classifies snaps and tanks and selects the legal timeout default", () => {
    ...
    expect(
      timeoutAction({
        ...baseContext,
        legal: { ...baseContext.legal, types: ["check", "bet"] },
      }).type,
    ).toBe("check");
  });
  it("paces most visible bot actions like human decisions", () => {
    ...
    const samples = Array.from(
      { length: 500 },
      () =>
        sampleAgentDecisionTiming(profile, baseContext, { type: "call" }, rng)
          .decisionTimeMs,
    );
    const subSecond =
      samples.filter((sample) => sample < 1_000).length / samples.length;
    const average =
  ...
  it("shows paced opponent turns for 500ms while preserving their simulated timing", () => {
    ...
    for (let guard = 0; guard < 500 && !checkedOpponent; guard++) {
      ...
      if (step.kind === "opponent-acting") {
        ...
        expect(step.engine.actions.at(-1)?.decisionTimeMs).toBe(
          step.decisionTimeMs,
        );
  ...
  it("puts elapsed timing into actions and the next opponent context", () => {
    ...
    tracker.apply(
      ...
      { decisionTimeMs: 420, timedOut: false },
    );
  ...
  it("learns different actions after snap and tank timing cues", () => {
    ...
    expect(
      BehavioralPolicy.deserialize(legacy).sample(
        baseContext,
        new Rng("legacy"),
      ).decisionTimeMs,
  ...
  it("records virtual timing in fast batch simulation without waiting", async () => {
    ...
    const voluntaryActions = output.hands.flatMap((hand) =>
      hand.actions.filter(
        (action) => action.type !== "post-sb" && action.type !== "post-bb",
      ),
    );
    expect(voluntaryActions.length).toBeGreaterThan(0);
    expect(
      voluntaryActions.every((action) => action.decisionTimeMs !== undefined),
    ).toBe(true);
    expect(
      voluntaryActions.every(
        (action) => (action.decisionTimeMs ?? 0) <= ACTION_CLOCK_MS,
      ),
    ).toBe(true);
    expect(
      output.userDecisionLog.every(
        (decision) => decision.decisionTimeMs !== undefined,
      ),
    ).toBe(true);
    expect(
      output.userDecisionLog.every(
        (decision) => decision.context?.holeCards.length === 2,
      ),
    ).toBe(true);
    expect(
      output.userDecisionLog.every(
        (decision) => decision.actionIndex !== undefined,
      ),
    ).toBe(true);
  });
